...
def _add_prose_requirement(conn: Connection, world: dict[str, Any]) -> uuid.UUID:
    """One ADMISSION_REQUIREMENT candidate, whose value is prose.

    `ADMISSION_REQUIREMENT` is in `NEVER_GROUPED`: a requirement's value is free text, so
    no component of it could make two requirements the same requirement.
    """
    candidate_id = uuid.uuid4()
    raw = "There are many ways that you can apply to study here."
    conn.execute(
        text(
            "INSERT INTO field_claim_candidate (id, extraction_id,"
            " pilot_collected_source_id, source_responsibility, field_kind,"
            " value_normalized, value_raw_text, evidence_text, locator, extractor_name,"
            " extractor_version, confidence_band, confidence_reason, claim_fingerprint)"
            " VALUES (:i, :e, :p, 'UNDERGRADUATE_ADMISSIONS', 'ADMISSION_REQUIREMENT',"
            " CAST(:v AS jsonb), :raw, :raw, CAST(:loc AS jsonb),"
            " 'admission-rule-extractor', '6', 'LOW', 'fixture band', :fp)"
        ),
        {
            "i": candidate_id,
            "e": world["ids"]["extraction"],
            "p": world["ids"]["pilot"],
            "v": _json({"requirement_text": raw}),
            "raw": raw,
            "loc": '{"heading_path": ["Entry pathways"]}',
            "fp": sha(raw),
        },
    )
    return candidate_id
...
def test_a_prose_requirement_reports_not_comparable_and_blocks_nothing(
    conn: Connection, world: dict[str, Any]
) -> None:
    """Found by walking the running console against the real ANU data.

    Four of ANU's eight candidates are prose requirements, and every one of them reported
    an `UNRESOLVED_CONFLICT` blocker. There was no conflict: `ADMISSION_REQUIREMENT` is
    never grouped, so "unresolved" described a question that was never asked. Agreement
    does not apply to prose, so it must not stand between a requirement and publication.

    The row is still gated -- by review state, source eligibility and scope -- which is
    the point: dropping a meaningless blocker is not the same as dropping a real one.
    """
    candidate_id = _add_prose_requirement(conn, world)
    item = next(
        i
        for i in candidate_resolution.queue(conn, institution=world["match_key"])
        if i.candidate_id == candidate_id
    )
    assert item.group_verdict == "NOT_COMPARABLE"
    assert item.conflict_resolved is True
    assert "UNRESOLVED_CONFLICT" not in item.blockers
    # The gates that do apply are untouched.
    assert "SCOPE_UNRESOLVED" in item.blockers
    assert item.blockers, "a prose requirement is still gated, just not by agreement"


def test_a_not_comparable_group_is_still_visible_to_a_reviewer(
    conn: Connection, world: dict[str, Any]
) -> None:
    """Not blocking is not the same as not shown.

    "These were never compared" is a fact about the evidence, and a reviewer should be
    able to read it rather than infer it from an absence in a list.
    """
    candidate_id = _add_prose_requirement(conn, world)
    groups = candidate_resolution.conflict_groups(conn, institution=world["match_key"])
    mine = [g for g in groups if any(m["candidate_id"] == str(candidate_id) for m in g["members"])]
    assert len(mine) == 1, "the prose requirement must appear in the group listing"
    assert mine[0]["verdict"] == "NOT_COMPARABLE"
    assert mine[0]["needs_resolution"] is False
    assert mine[0]["resolved"] is True


def test_every_blocked_candidate_has_a_group_a_reviewer_can_open(
    conn: Connection, world: dict[str, Any]
) -> None:
    """The regression guard for the dead end itself, stated generally.

    `conflict_groups` can only list a group that has a fingerprint, and
    `candidate_conflict_resolution` can only store one against a fingerprint. So a queue
    item that claims `UNRESOLVED_CONFLICT` while carrying no fingerprint is a blocker with
    no remedy anywhere in the system -- not in the console, not in the CLI, not in the
    table. Nulling the fingerprint for unkeyable rows produced exactly that.

    Asserting it over the whole queue rather than one row is deliberate: the next field
    kind that cannot be keyed will be caught here without anyone remembering to add a
    case.
    """
    _add_prose_requirement(conn, world)
    items = candidate_resolution.queue(conn, institution=world["match_key"])
    listed = {
        g["context_fingerprint"]
        for g in candidate_resolution.conflict_groups(conn, institution=world["match_key"])
    }
    assert items
    for item in items:
        assert item.context_fingerprint is not None, f"{item.field_kind} has no group identity"
        assert (
            item.context_fingerprint in listed
        ), f"{item.field_kind} names a group the reviewer cannot open"
        if "UNRESOLVED_CONFLICT" in item.blockers:
            assert item.context_fingerprint in listed

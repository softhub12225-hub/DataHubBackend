...
def test_a_calendar_event_is_never_grouped_for_agreement() -> None:
    ...
    assert groups[0].verdict is Agreement.NOT_COMPARABLE


def test_a_never_grouped_field_is_not_comparable_rather_than_thin() -> None:
    """The two unkeyable outcomes must stay distinguishable.

    Both `NOT_COMPARABLE` and `INSUFFICIENT_CONTEXT` arrive here as singletons with no
    real context key, and reporting them identically cost the console a blocker nobody
    could clear: a prose requirement claimed an unresolved conflict, while the group
    listing -- which only lists keyable groups -- offered nothing to resolve.

    The distinction is about the *field*, not the row: prose will never key, whereas a
    tuition figure with no currency would key if the page had stated one.
    """
    prose = group_candidates(
        [
            row(
                field_kind=FieldKind.ADMISSION_REQUIREMENT.value,
                responsibility="UNDERGRADUATE_ADMISSIONS",
                value={"requirement_text": "There are many ways to apply to ANU."},
            )
        ]
    )
    assert prose[0].verdict is Agreement.NOT_COMPARABLE

    # A groupable field whose row is missing a required component is the other case.
    thin = group_candidates(
        [
            row(
                field_kind=FieldKind.TUITION.value,
                responsibility="TUITION_FEES",
                value={"amount": 48000},  # no currency, billing_unit or student_category
            )
        ]
    )
    assert thin[0].verdict is Agreement.INSUFFICIENT_CONTEXT
    assert FieldKind.TUITION.value not in NEVER_GROUPED

...
def load(store: EvidenceStore, evidence: BodyEvidence) -> NormalizedDocument:
    ...
    A store that does not hold the hash raises `SourceBodyError`, not the `KeyError` the
    store itself raises. Found by opening the evidence viewer against real ANU data with a
    mistyped `ARTIFACT_ROOT`: the blob store's `KeyError` travelled all the way to the
    browser as HTTP 500 "An unexpected error occurred", so the one screen whose job is to
    show a reviewer what the evidence says instead showed nothing and explained nothing.

    "The database names a document this store does not have" is a legitimate operational
    state -- a wrong root, a pruned cache, an artifact directory that was never copied --
    and a reviewer must be told that rather than shown a stack trace. Translating it here
    rather than at the route means the CLI gets the same answer.
    """
    ...
    try:
        ...
        raise SourceBodyError(
            f"{NOT_AVAILABLE}: {evidence.source_ref} names document "
            f"{evidence.document_hash[:12]}..., which this evidence store does not hold"
        ) from exc

...
__all__ = [
    ...
    # Re-exported deliberately, not incidentally. Callers that need to reproduce a
    # manifest digest must use the function THIS module verifies against: a second
    # implementation is how a check starts passing against a hash nobody else would
    # produce, which happened for real while verifying the domain manifest. Naming it
    # here is what makes `responsibility_binding.manifest_digest` a supported import

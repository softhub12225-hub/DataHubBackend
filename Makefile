...
#
# Every target here is also reachable as a direct uv command
...
.PHONY: help bootstrap up up-deps down down-volumes logs ps \
        ...
        lint format typecheck test test-unit export-openapi check-manifests \
...
# Setup
# ---------------------------------------------------------------------------
bootstrap: ## Install dependencies and create .env if absent
...
# Local stack
# ---------------------------------------------------------------------------
up: ## Start the backend stack (postgres, redis, minio, api, worker, beat)
	$(COMPOSE) up --build -d
	@echo "api  -> http://localhost:8000/health/ready"
	@echo "the console is a separate repository; point its API_BASE_URL at this API"
...
test: ## Run the tests (integration tests skip without live services)
	$(TEST_DB_ENV) $(UV) run pytest

test-unit: ## Run the tests excluding integration
	$(TEST_DB_ENV) $(UV) run pytest -m "not integration"

check: lint typecheck test check-manifests ## Everything CI runs

check-manifests: ## Assert the committed manifests match their pinned sha256
	cd apps/api && $(UV) run python -m scripts.check_manifests
...
# API contract
# ---------------------------------------------------------------------------
export-openapi: ## Write this API's OpenAPI document for the console repo
	$(UV) run python -m scripts.export_openapi
	@echo "commit the document in the console repo, then run its generate:api-types"
